...
class notifica(models.Model):
    ...
    def recalculaDias(self, dia_inicio, cant_dias, tipo):
        ...
        d = d_full.split('T')[0]
        cont_dias = 0
        print (("DIA DE INICIO PARA REALIZAR EL CALCULO: " + str(d)))
    ...
    def prueba_envio_mensaje(self):
        # message_post(self, body='', subject=None, message_type='notification', subtype=None, parent_id=False,
        #              attachments=None, content_subtype='html', **kwargs)
        # https: // www.odoo.com / documentation / 11.0 / reference / mixins.html
        self.message_post(body="Mensaje de Prueba 2021 b", subject=None, message_type='notification', subtype='mt_comment')
        return True
    ...
    def eliminar_alertas(self):
        ...
        expte_obj = self.browse([active_id])
        lista_subscripcion = []
        print (("ELIMINANDO A USUARIOS QUE SIGUEN ESTE EVENTO"))
        grupos = expte_obj.plazo_id.grupos_notificar
        dia_vencimiento = expte_obj.fecha_vencimiento
        user_creador_id = expte_obj.user_creador_id.id
        for g in grupos:
            for u in g.user_notificar_id:
                print (("Generando ELIMINADION para Empleado: " + u.name + " ID Users: "+ str(u.user_id.id) + " Usuario: "+ str(u.user_id.login)+ "Usuario Activo: " + str(u.active)))
                lista_subscripcion.append(u.user_id.id)
        if user_creador_id not in lista_subscripcion:
            lista_subscripcion.append(user_creador_id)
        print(("LA LISTA DE DES SUBSCRIPCION CREADA ES:" + str(lista_subscripcion)))
        self.message_unsubscribe_users(user_ids=lista_subscripcion)
        return True
...
class expediente(models.Model):
    ...
    def plazos_asociados(self):
        ...
        if not depart_user_actual_id:
            print(("No hay oficina actual asignada."))
        if expte_obj.tarea_actual.departament_id.id != depart_actual_expte_id.id:
            print(("No pertenece a la misma oficina que la tarea."))
            raise ValidationError(
                ('Solo puede administrar los plazos asociados a esta tarea si el documento se encuentra en ' + str(expte_obj.tarea_actual.departament_id.name)))
        if depart_user_actual_id != depart_actual_expte_id.id:
            print(("No pertenece a la misma oficina que el expediente."))
            raise ValidationError(
        ...
        if len(legal_list) > 0:
            ...
            'views': [[self.env.ref('notificaciones.plazo_list_con_opcion').id, "tree"]],
            'target': 'new',
            }
        else:
