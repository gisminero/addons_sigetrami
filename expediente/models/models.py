...
class exp_mineral(models.Model):
    ...
    categoria_mineral_exp = fields.Selection([
        ('primera', 'Primera'),
        ('segunda', 'Segunda'),
        ('tercera', 'Tercera'),], required=False, readonly=True,
        help="Categoria del mineral")
    lugar_donde = fields.Selection([
        ('1', 'Lechos de Rios'),
        ('2', 'Aguas Corrientes'),
        ('3', 'Placeres'), ], required=False,
        help="Categoria del mineral")
    otro_mineral = fields.Char('Otro Mineral', required=False) # Se habilita Solo en 3 categoria
    forma_presentacion = fields.Char('Solicitante', required=False)
...
class exp_pertenencias(models.Model):
    ...
    _description = "Historial de Pertenencias"

    cant_pertenencias_canon = fields.Integer('Cantidad de Pertenencias', help='', default=0)
...
class expediente(models.Model):
    ...
    def depart_user(self):
        ...
        if id_depart == False:
            raise ValidationError(('Configurar Empleado: Debe asociar una oficina o departamento al usuario actual A'))
        return id_depart
    ...
    recibido = fields.Boolean('Recibido', readonly=True, store=True, default=False)#compute=_is_recept_doc
    ...
    def enviar(self):
        ...
        if self.recibido:
            ...
            if not expte_obj.oficina_destino.name:
                ...
                # Probando la misma linea sin unidecode 07/07/2021
